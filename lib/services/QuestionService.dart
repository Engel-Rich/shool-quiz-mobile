  ...
  Future<List<QuestionModel>> questionByCatId(String? catId) async {
    if (catId != null) {
      ...
      return await ref.get().then((value) {
        return value.docs.map((e) => QuestionModel.fromJson(e.data() as Map<String, dynamic>)).toList();
      });
    }
  }

  Future<List<QuestionModel>> questionByType(String? type) async {
    return ref.where("questionType", isEqualTo: type).get().then((res) {
      if (res.docs.isNotEmpty) {
        return res.docs.map((e) => QuestionModel.fromJson(e.data() as Map<String, dynamic>)).toList();
      } else {
        toast('Quiz is not available');
        throw 'Not available';
      }
